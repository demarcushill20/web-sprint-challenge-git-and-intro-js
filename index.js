...
console.log(artists); // output: the corrected dataset with Vincent van Gogh's name fixed
...
function listOfNames(artists) {
  // Create a new array with the same length as the artists array
  const names = new Array(artists.length);
  
  // Loop over the artists array and update the names array
...
function addArtist(artistsArr, name, years, genre, nationality, bio) {
  ...
  // Add the new object to the copied array
  copiedArr.push(newArtist);

  // Return the copied array with the new object added
  return copiedArr;
}

const updatedArtists = addArtist(
  artists,
  'Demarcus Hill',
  '1991 - current day',
  'Web Design',
  'African-American',
  'I am a student at Bloomtech University'
);
...
function lotsOfArt(artists) {
  // Use filter method to filter out artists with less than or equal to 100 paintings
  const manyPaintings = artists.filter(artist => artist.paintings > 100);
  
  // Use map method to return an array with names of remaining artists
  const names = manyPaintings.map(artist => artist.name);
...
function artistInfo(artists, name) {
  ...
  for (let i = 0; i < artists.length; i++) {
    // Check if the name of the current artist matches the input name
    if (artists[i].name === name) {
      // If so, return the artist's bio
      return artists[i].bio;
    }
  }
  // If no artist is found with the input name, return null
  return null;
}
...
function artistByCountry(artistArr, nationality) {
  // Filter artists by nationality and exclude artists with multiple nationalities
  const filteredArtists = artistArr.filter(artist => artist.nationality === nationality && artist.nationality.split(',').length === 1);

  // Map over the filtered array to only return artist names
